...
void loop()
{
    ...
    if (Serial.available())
    {
        ...
        case '1':
            playAudio("/M8b24kM.wav");
            break; //Play a *.wav file by name - 16bit, 32khz, Mono
        case '2':
            playAudio("/M8b24kS.wav");
            break; //Play  16bit, 32khz, Stereo
        case '3':
            playAudio("/M8b44kST.wav");
            break; //Play 16bit, 44khz, Mono
        case '4':
            playAudio("/M16b24kS.wav");
            break; //Play  16bit, 44khz, Stereo
        ...
        case 't':
            useTasks = !useTasks;
            Serial.print("Using Tasks: ");
            Serial.println(useTasks ? "True" : "False");
            break;
        case 'e':
            adcReadings = 32;
            break;
        case 'r':
            adcReadings = MAX_BUFFER_SIZE;
            break;
        case 'a':
            Serial.println("Manual Close");
            aaAudio.rampOut(0);
            aaAudio.disableDAC();
            #pragma message "o que fazer com isso?"
            // myFile.close();
            break;
        case 's':
            Serial.println("Playing 8-bit sine wave");
            aaAudio.setSampleRate(32000);
            aaAudio.dacBitsPerSample = 8;
            sineSetup8();
            doSine = true;
            break;
        case 'd':
            Serial.println("Playing 16-bit sine wave");
            aaAudio.setSampleRate(32000);
